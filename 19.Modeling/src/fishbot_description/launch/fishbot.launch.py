...
def generate_launch_description():
    ...
    # default_model_path = urdf_tutorial_path + "/urdf/fishbot/fish.urdf.xacro"
    # default_rviz_config_path = urdf_tutorial_path + '/config/rviz/fishbot_model.rviz'
    ...
    # Command(['xarro ', 路径])：执行系统 shell 命令xarro，xarro 路径的作用是读取文件的文本内容，这是 ROS2 中加载文件内容的标准写法；
    # ParameterValue(内容, value_type=str)：将xarro命令读取的 URDF 文本内容，封装为字符串类型的 ROS2 参数值（因为 URDF 是文本，必须指定str类型）；
    # parameters=[{'robot_description': robot_description}]：给robot_state_publisher节点传 URDF 模型，是该节点计算机器人坐标系的核心依据，必须传；
